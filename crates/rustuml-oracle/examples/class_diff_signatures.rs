...
//! Print first few diff signatures for class golden failures.
//! Usage: cargo run --release -p rustuml-oracle --example class_diff_signatures -- <max>
...
fn main() {
    ...
    for (sig, count) in sigs.iter().take(30) {
        println!("{:5}  {}", count, sig);
